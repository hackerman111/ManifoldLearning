...
\begin{EQA}[rcl]
	&& \nquad
	\arginf_{\betav} \biggl\{ \sum_{j \in \JJJ} \bigl\| \Imav_{j} - \fcn_{j} \Sv_{j} - \fcl_{j} \, \UV_{j} \betav \bigr\|^{2}
	+ \lambda \| \betav - \betav_{0} \|^{2} 
	\biggr\}	
	\\
	&=&
	\biggl\{ \sum_{j \in \JJJ} \fcl_{j}^{2} \, \UV_{j}^{\T} \UV_{j} + \lambda \Id_{\dimd} \biggr\}^{-1}
	\sum_{j \in \JJJ} \bigl\{ \fcl_{j} \UV_{j} (\Imav_{j} - \fcn_{j} \Sv_{j}) 
	+ \lambda \betav_{0}
	\bigr\}
\end{EQA}
{\color{blue} please check}
...
\item 
Initialize \( \betav_{0} \) by a prior information or other procedure like PCA.
Set \( m=1 \).
\item 
...
\begin{EQA}[c]
	(\fcn_{j}^{(m)},\fcl_{j}^{(m)})
	=
	\arginf_{\fcn_{j},\fcl_{j}} \bigl\| \Imav_{j} - \fcn_{j} \Sv_{j} - \fcl_{j} \, \UV_{j} \betav \bigr\|^{2}
\end{EQA}
...
We follow the structure-adaptive approach from \cite{}, which applies anisotropic weights 
...
to ensure sufficiently many design points in a local vicinity of \( \xv_{j} \).
...
\item 
the value \( \haniso_{\min} > 1 \); par defaut, \( \haniso_{\min} = 10 \sigmaX/n \).
\end{mylist}
...
\begin{EQA}[c]
	...
	\qquad
\end{EQA}
by alternating optimization; see Section~\ref{SLLaltern}.
...
\label{jhgiverkytde4w34etycja}
\end{EQA}
and fix \( \TEDR_{k} \) in the form \eqref{iufhvu8kjbmnll475edf} with \( \betav = \betav_{k-1} \),
...
\paragraph{Loop}
%
Increase \( k \) and loop until \( \haniso_{k}/a < \haniso_{\min} \).
...
\end{EQA}
For any direction \( \sph \in \Sph_{j} \), compute 
\begin{EQ}[rcccl]
	\Ima_{j,\sph}
	&=&
	\sumi Y_{i} \langle \Xv_{i} - \xv_{j} \, ,\sph \rangle \, \weight_{ij} 
...
The class of such mappings is not convex, this makes the problem computationally hard.
We relax this condition and optimize w.r.t. \( \BEDR = (\betav_{1},\ldots,\betav_{\dimm})^{\T} \in \R^{\dimm \times \dimd} \). 
Then the desired orthogonal mapping \( \PEDR \) will be obtained using SVD of the solution \( \BEDR \).
%
To ensure identifiability, one can add a penalty term \( \lambda \| \BEDR - \PEDR_{0} \|_{\Fr}^{2} \)
for a preliminary guess \( \PEDR_{0} \).
This leads to
\begin{EQA}
	\LL_{\lambda}(\prmtv)
	&=&
...
One can apply the alternating optimization procedure of Section~\ref{SLLaltern}.
 
If a preliminary guess \( \PEDR_{0} \) is not available, or if \( \lambda = 0 \),
the minimization problem \eqref{gtgyuy6ew5dtde32322jtmiG0} may have many solutions; any of them can be taken:
\begin{EQA}[c]
%	\prmtv_{0}
%	= 
	\{ \BEDR_{0} \, , (\fcn_{j}^{(0)},\fclv_{j}^{(0)})_{j \in \JJJ} \} 
	=
	\arginf_{\BEDR \, , (\fcn_{j},\fclv_{j})_{j \in \JJJ}} \LL(\prmtv)
	\, .
...
\( \Imav_{j} \), \( \Sv_{j} \), and \( \UV_{j} \) is suboptimal and and to poor 
quality of recovering the EDR-space \( \EDR \).
The structure-adaptive approach from \cite{HJPS} suggests to exctract the structural information from the current-step estimate 
...
the structural multi-index information about the model
...
\begin{EQA}[c]
	...
	\sum_{j \in \JJJ} \nabla \fs(\xv_{j}) \, \nabla \fs(\xv_{j})^{\T}
	=
	\sum_{j \in \JJJ} \PEDR \fclv_{j} \, \bigl( \PEDR \fclv_{j} \bigr)^{\T}
%	\biggl( \sum_{m=1}^{\dimm} \fcl_{j,m} \, \betav_{m} \biggr)^{\T}
	=
	\PEDR \biggl( \sum_{j \in \JJJ} \fclv_{j} \, \fclv_{j}^{\T} \biggr) \PEDR^{\T}
	\, 
\end{EQA}
accumulates the structural information about the model.
A starting solution \( \BEDR_{0} \) from \eqref{ju7fudeju74334fhfd} can be used 
to compute a starting estimate \( \JEDR_{0} \) of \( \JEDR \) by
\begin{EQA}
	\JEDR_{0}
	\eqdef 
	\sum_{j \in \JJJ} (\BEDR_{0}^{\T} \fclv_{j}^{(0)}) \, (\BEDR_{0}^{\T} \fclv_{j}^{(0)})^{\T}
	&=&
	\BEDR_{0}^{\T} \Bigl( \sum_{j \in \JJJ} \fclv_{j}^{(0)} \, {\fclv_{j}^{(0)}}^{\T} \Bigr) \BEDR_{0}
	\, .
\label{uhehdisnckncw9ud9}
\end{EQA}
As \( \BEDR_{0} \) is of rank \( \dimm \), the same holds for \( \JEDR_{0} \).
...
\begin{EQA}[c]
	...
	\; ,
\end{EQA}
where \( \Lambda_{0} \) is a diagonal \( \dimm \)-matrix with decreasing eigenvalues.

Now we try to improve the procedure by redefining the tensor \( \TEDR \) as
\begin{EQA}
	\TEDR^{2}
	&=&
	\hiso^{-2} \bigl( \aniso_{k}^{2} \, \Id_{\dimd} + \JEDR_{0} \bigr)
...
minimization of \( \LL_{\lambda}(\prmtv) \) from \eqref{gtgyuy6ew5dtde32322jtmiG}
after this update.
%
...
\begin{EQA}[c]
	\{ \BEDR_{0} \, , (\fcn_{j}^{(0)},\fclv_{j}^{(0)})_{j \in \JJJ} \} 
	= 
	\arginf_{\BEDR \, , (\fcn_{j},\fclv_{j})_{j \in \JJJ}} 
	\sum_{j \in \JJJ} \bigl\| \Imav_{j}^{(0)} - \fcn_{j} \Sv_{j}^{(0)} - \UV_{j}^{(0)} \, \BEDR^{\T} \fclv_{j} \bigr\|^{2}
	\, .
	\qquad
...
\item 
Compute \( \Ima_{j,\sph}^{(k)} \, , \, S_{j,\sph}^{(k)} \, , \, \Uv_{\jm,\sph}^{(k)} \) by \eqref{ytd7e7ydwuhd5eythdm} 
with \( \weight_{ij} = \Kern\bigl( \TEDR_{k} (\Xv_{i} - \xv_{j}) \bigr) \)
for all \( j \in \JJJ \), \( \sph \in \Sph_{j} \).
...
solve
\begin{EQA}[c]
	(\BEDR_{k},(\fcn_{j}^{(k)},\fclv_{j}^{(k)})_{j \in \JJJ})
	=
	\arginf_{\BEDR,(\fcn_{j},\fclv_{j})_{j \in \JJJ}}
	\sum_{j \in \JJJ} \bigl\| \Imav_{j}^{(k)} - \fcn_{j} \Sv_{j}^{(k)} - \UV_{j}^{(k)} \, \BEDR^{\T} \fclv_{j} \bigr\|^{2} 
	+ \lambda \| \BEDR - \PEDR_{k-1} \|_{\Fr}^{2}
	\, ,
\end{EQA}
where \( \BEDR = (\betav_{1}, \ldots, \betav_{\dimm})^{\T} \in \R^{\dimm \times \dimd} \), %\PROJ_{\dimd, \dimm} \), 
...
\item
Loop while \( \hiso_{k} \geq \hiso_{\min}/a \).
...
The manifold hypothesis extends the multi-index approach by allowing the mapping \( \PEDR \) also to depend on \( \xv \) but slowly.
Namely, we assume that for close points \( \xv \) and \( \xvd \), the corresponding mappings 
\( \PEDR(\xv) \) and \( \PEDR(\xvd) \) are close as well. 
Equivalently, for any point \( \xvd \), in a relatively large its vicinity, it holds \( \PEDR(\xv) \approx \PEDR(\xvd) \).
%However, a more common assumption is that the curvature (Hessian) of the manifold varies slowly.
%This means that the subspace spanned by \( \PEDR_{j}^{\T} = \PEDR(\xv_{j})^{\T} \) very lightly changes over \( \xv_{j} \)
%in a relatively large vicinity of a fixed point \( \xvd \). 
%Each subspace \( \EDR_{j} \) is \( \dimm \)-dimensional, 
%and its linear change yields that all \( (\EDR_{j}) \) are within one \( 2\dimm \)-dimensional
%subspace spanned by \( (\PEDR_{\jc}^{\T}) \) . 
%
...
The major difference is in using two localizing schemes described by 
localizing tensors \( \TEDR \) and \( \TEDRm \) and the corresponding localizing weights.

Suppose a collection of centers \( \xv_{j} \), and, for each \( j \in \JJJ \),
the set \( \Sph_{j} \) of directions \( \sph \) in \( \R^{\dimd} \) have been fixed.
% 
Further, let 
...
%	\prmtv^{(0)} 
	&& \nquad
	\bigl\{ \BEDR_{\jl} \, , (\fcn_{\jjl},\fclv_{\jjl})_{j \in \JJJ} \bigr\}
	= 
	\arginf_{\BEDR \, , (\fcn_{j},\fclv_{j})_{j \in \JJJ}} 
	\sum_{j \in \JJJ} 
	\bigl\| \Imav_{j} - \fcn_{j} \Sv_{j} - \UV_{j} \BEDR^{\T} \fclv_{j} \bigr\|^{2}
	\weightM_{\jjl}
...
\( \weightM_{\jjl} \)
%\( \Kern\bigl( \| \TEDRm(\xv_{j} - \xv_{\jl}) \|^{2} \bigr) \)
for each entry \( \bigl\| \Imav_{j} - \fcn_{j} \Sv_{j} - \UV_{j} \BEDR^{\T} \fclv_{j} \bigr\|^{2} \).
...
\end{EQA}
over \( \{ \BEDR_{\jl} \, , (\fcn_{\jjl},\fclv_{\jjl})_{j \in \JJJ} \}_{\jl \in \JJJ} \).
%
%
The choice of the family \( (\BEDR_{\jl}) \) can be stabilized and synchronized by the penalty
\begin{EQA}[c]
	\lambda_{\PEDR} \, \pent_{\PEDR}(\BEDR)
	\eqdef
	\lambda_{\PEDR} \sum_{\jl \in \JJJ} \sum_{j \in \JJJ} \| \BEDR_{j} - \BEDR_{\jl} \|_{\Fr}^{2} 
	\; \weightM_{\jjl}
...
\( \Sv_{j}^{(0)} = \bigl( S_{j,\sph}^{(0)} \bigr)_{\sph \in \Sph_{j}} \),
\( \UV_{j}^{(0)} = (\Uv_{\jm,\sph}^{(0)} \, , \sph \in \Sph_{j})^{\T} \),
solve for every \( \jl \in \JJJ \)
\begin{EQA}[rcl]
	&& \nquad
	\{ \BEDR_{\jl}^{(0)} \, , (\fcn_{\jjl}^{(0)},\fclv_{\jjl}^{(0)})_{j \in \JJJ} \} 
	\\
	&=& 
	\arginf_{\BEDR \, , (\fcn_{j},\fclv_{j})_{j \in \JJJ}} 
	\sum_{j \in \JJJ} 
	\bigl\| \Imav_{j}^{(0)} - \fcn_{j} \Sv_{j}^{(0)} - \UV_{j}^{(0)} \BEDR^{\T} \fclv_{j} \bigr\|^{2}
...
\end{EQA}
%where the term \( \lambda_{\PEDR} \pent(\BEDR) \) penalizes for local variability of \( \BEDR_{j} \)
%as a function of \( \xv_{j} \):

\item
With \eqref{kjhviir8786tytcdeef} solved, compute the matrices \( \JEDR_{\jl}^{(0)} \)
\begin{EQA}[rcl]
	\JEDR_{\jl}^{(0)}
	& \eqdef &
	\sum_{j \in \JJJ} ({\BEDR_{\jl}^{(0)}}^{\T} {\fclv}_{\jjl}^{(0)}) \, \bigl( {\BEDR_{\jl}^{(0)}}^{\T} {\fclv}_{\jjl}^{(0)} \bigr)^{\T}
	\Kern\bigl( \hisom_{0}^{-2} \| \xv_{j} - \xv_{\jl} \|^{2} \bigr)
	\\
	&=&
	{\BEDR_{\jl}^{(0)}}^{\T} \Bigl\{
		\sum_{j \in \JJJ} {\fclv}_{\jjl}^{(0)} \, ({\fclv}_{\jjl}^{(0)})^{\T} 
		\Kern\bigl( \hisom_{0}^{-2} \| \xv_{j} - \xv_{\jl} \|^{2} \bigr)
	\Bigr\} \BEDR_{\jl}^{(0)}
\end{EQA}
and their SVD representation
\begin{EQA}[c]
	\JEDR_{\jl}^{(0)}
	=
	{\PEDR_{\jl}^{(0)}}^{\T} \Lambda_{\jl}^{(0)} \, \PEDR_{\jl}^{(0)} 
	\, , 
	\qquad
	\PEDR_{\jl}^{(0)} \, {\PEDR_{\jl}^{(0)}}^{\T}
	=
	\Id_{\dimm}
	\, .
\end{EQA}
...
\( (\BEDR_{\jl}^{(k-1)} \, , \JEDR_{\jl}^{(k-1)} )_{\jl \in \JJJ} \) be available.
\begin{mylist}
...
\begin{EQA}[rcl]
	...
	(\TEDRm_{\jl}^{(k)})^{2}
	&=&
	\frac{\aniso_{k}^{2}}{\hisom_{k}^{2}} (\Id_{\dimd} - {\PEDR_{\jl}^{(k-1)}}^{\T} \PEDR_{\jl}^{(k-1)}) 
	...
	\, ,
\end{EQA}
where the anisotropy index \( \aniso_{k} \) is the largest value ensuring
\begin{EQA}[rcl]
...
\begin{EQA}[rcl]
	...
	\weight_{ij}^{(k)} 
	&=& 
	\Kern\bigl( \TEDR_{j}^{(k)} (\Xv_{i} - \xv_{j}) \bigr) 
	\, ,
	\qquad
	i = 1,\ldots,n
	\, ,
...
\begin{EQA}[rcl]
	&& \nquad
	\{ \BEDR_{\jl}^{(k)} \, , (\fcn_{\jjl}^{(k)},\fclv_{\jjl}^{(k)})_{j \in \JJJ} \}_{\jl \in \JJJ}
	\\
	&=& 
	\arginf_{\{ \BEDR_{\jl} \, , (\fcn_{\jjl},\fclv_{\jjl})_{j \in \JJJ} \}_{\jl \in \JJJ}} 
	\biggl\{ \sum_{\jl \in \JJJ} \sum_{j \in \JJJ} 
		\bigl\| \Imav_{j}^{(k)} - \fcn_{\jjl} \Sv_{j}^{(k)} - \UV_{j}^{(k)} \BEDR^{\T} \fclv_{\jjl} \bigr\|^{2}
		\Kern\bigl( \| \TEDRm_{\jl}^{(k)} (\xv_{j} - \xv_{\jl}) \|^{2} \bigr)
	\\
	&& \qquad
		+ \, \lambda \| \BEDR - \PEDR^{(k-1)} \|_{\Fr}^{2} + \lambda_{\PEDR} \pent(\BEDR)
	\biggr\}
	\, ,
...
\begin{EQA}[c]
	\pent(\BEDR)
	\eqdef
	\sum_{\jl \in \JJJ} \sum_{j \in \JJJ} \| \BEDR_{j} - \BEDR_{\jl} \|_{\Fr}^{2} 
	\;
	\Kern\bigl( \| \TEDRm_{\jl}^{(k)} (\xv_{j} - \xv_{\jl}) \|^{2} \bigr)
	\, .
\end{EQA}
...
\begin{EQA}[c]
	...
	\, .
\end{EQA}
If \( \hiso_{k}/a \geq \hiso_{\min} \), loop.
\end{mylist}
...
\end{EQA}
over all \( \betav \in \R^{\dimd} \) and \( (\fcl_{j})_{j \in \JJJ} \in \R^{\nSph} \), 
which can be solved by alternating minimization as for \eqref{ikdfjoiefjoieuvbjmr3e}.
For \( \betav \) fixed, each \( \fcl_{j} \) is estimated by
\begin{EQA}[c]
	\hat{\fcl}_{j}
	=
	\frac{\langle \Imav_{j}, \UV_{\jm} \, \betav \rangle}{\| \UV_{\jm} \, \betav \|^{2}}
	\, ,
\end{EQA}
while for given \( (\fcl_{j})_{j \in \JJJ} \), the vector \( \betav \) can be estimated by
\begin{EQA}[c]
	\hat{\betav}
	=
	\biggl( \sum_{j \in \JJJ} \fcl_{j}^{2} \; \UV_{\jm}^{\T} \UV_{\jm} + \lambda \Id_{\dimp} \biggr)^{-1}
	\biggl( \sum_{j \in \JJJ} \fcl_{j} \, \UV_{\jm}^{\T} \Imav_{j} + \lambda \betav_{0} \biggr)
	\, .
\end{EQA}
{\color{blue} please check}
...
\end{EQA}
\item 
Find \( \betav_{0},(\fcl_{j}^{(0)})_{j \in \JJJ} \) by minimizing \eqref{ikdfjoiefjoieuAVE} using alternating minimization.
\item 
%:
rescale \( \betav_{0} \) by \( \| \betav_{0} \|^{-1} \) and \( (\fcl_{j}^{(0)}) \) by \( \| \betav_{0} \| \) to ensure 
\( \| \betav_{0} \| = 1 \).
...
\end{EQA}
Later, we relax the constraint \( \PEDR \in \PROJ_{\dimd,\dimm} \) by 
replacing \( \PEDR \) with \( \BEDR \in \R^{\dimm \times \dimd} \), and
add stabilization penalty \( \lambda \| \BEDR - \BEDR_{0} \|_{\Fr}^{2} \)
for a preliminary guess \( \BEDR_{0} \) leading to the problem
%
\begin{EQA}[c]
%	\sum_{j \in \JJJ} \sum_{\sph \in \Sph_{j}} 
...
Find 
\begin{EQA}[c]
	\{ \BEDR_{0} \, , (\fclv_{j}^{(0)})_{j \in \JJJ} \} 
	= 
	\arginf_{\BEDR \, , (\fclv_{j})_{j \in \JJJ}} 
	\sum_{j \in \JJJ} \bigl\| \Imav_{j} - \UV_{\jm} \, \BEDR^{\T} \, \fclv_{j} \bigr\|^{2}
	\, .
\end{EQA} 
Here, the solution is not unique; any minimizer can be used.  
%from \eqref{gtgyuy6ew5dtde32322jtmi by alternating minimization.
...
Solve by alternating minimization
\begin{EQA}[c]
	\{ \BEDR_{k},(\fclv_{j}^{(k)})_{j \in \JJJ} \} 
	=
	\arginf_{\BEDR,(\fclv_{j})_{j \in \JJJ}} 
	\sum_{j \in \JJJ} \bigl\| \Imav_{j}^{(k)} - \UV_{\jm}^{(k)} \, \BEDR^{\T} \fclv_{j} \bigr\|^{2}
