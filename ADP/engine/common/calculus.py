"""Общие геометрические, bandwidth и random-direction вычисления ADP."""
...
from . import utils
from .utils import _prepare_basis_eigenvalues, _prepare_multi_localization
from .weights import _multi_components, _projected_multi_components


def pairwise_distance2(X: np.ndarray, centers: np.ndarray) -> np.ndarray:
    """Посчитать ``||x_i-c_j||²`` через Gram-тождество.

    Данные проверяются, центрируются общим ``origin`` для устойчивости при
    больших сдвигах, затем вычисляется матрица ``(J, n)`` без тензора
    разностей ``(J, n, d)``. Формула используется весами и поиском bandwidth.
    """
    ...
    # NUMERICAL: Gram cancellation may leave a tiny positive self-distance.
    # Recompute only those candidates directly; do not materialize (J,n,d).
    scale = max(
        1.0,
        float(np.max(np.square(centers).sum(axis=1))),
        float(np.max(np.square(X).sum(axis=1))),
    )
    rows, columns = np.nonzero(distance2 <= 64.0 * np.finfo(float).eps * scale)
    for row, column in zip(rows, columns, strict=True):
        delta = centers[row] - X[column]
        distance2[row, column] = float(np.dot(delta, delta))
    return distance2
...
def search_bandwidth(
    ...
    *,
    ...
    """Найти минимальную ширину с заданной средней локальной массой.

    Верхняя граница расширяется удвоением, затем условие монотонной массы
    решается бисекцией. Для Epanechnikov поддержка сокращается только по
    доказанно нулевым элементам. Вход — ``distance2`` формы ``(J, n)``;
    выход — конечная ширина ``h``.
    """
    distance2 = utils._prepare_bandwidth(distance2, target, kernel, lower)
    utils.require(
        not (kernel is epanechnikov and target > distance2.shape[1]),
        "target mass cannot exceed n for the Epanechnikov kernel",
    )
    active = distance2.ravel()

    def enough(h: float) -> bool:
        """Проверить достижимость целевой массы при ширине ``h``."""
        nonlocal active
...
def calculate_h0(
    ...
    """Получить начальную bandwidth через локальную массу ``N_loc``.

    Функция объединяет расчёт попарных расстояний и ``search_bandwidth`` для
    единой точки входа алгоритма.
    """
...
def calculate_h0_from_adp(config, data) -> float:
    """Адаптировать поиск ``h0`` к полям legacy ``config`` и ``data``."""
...
def calculate_rho_k(
    ...
    *,
    ...
    """Найти максимальный anisotropy factor ``rho`` с нужной массой.

    Проекционная и ортогональная части расстояния вычисляются отдельно, а
    монотонное условие средней kernel-массы решается бисекцией на ``[0, 1]``.
    Варианты ``new`` и ``legacy`` соответствуют двум формулам локализации;
    при недостижимой массе возвращается ``None``.
    """
    ...
    utils.require(np.isfinite(N_loc) and N_loc > 0, "N_loc must be finite and positive")

    if distance2 is None:
        distance2 = pairwise_distance2(X, centers)
    distance2 = utils.require_not_none(distance2, "distance2 is required")
    ...
    def enough(rho: float) -> bool:
        """Проверить среднюю локальную массу при данном ``rho``."""
        if estimator == "new":
...
def calculate_rho_k_from_adp(config, data, beta, h_k) -> float | None:
    """Адаптировать поиск ``rho_k`` к legacy-контейнерам ADP."""
...
def generate_proj(
    ...
    """Сгенерировать локализованные случайные единичные направления.

    Нормальные векторы смешиваются с текущим индексом ``beta`` через ``rho``
    и нормируются по последней оси. Результат имеет форму
    ``(n_centers, n_directions, d)``.
    """
    ...
    utils.require(not np.any(norms == 0), "generated a zero direction", RuntimeError)
    return values / norms
...
def generate_isotropic_proj(
    ...
    utils.require(
        not isinstance(n_features, bool) and isinstance(n_features, (int, np.integer)),
        "n_features must be an integer",
        TypeError,
    )
    utils.require(n_features >= 1, "n_features must be positive")
...
def generate_proj_from_adp(config, data, rng, beta, rho) -> np.ndarray:
    """Адаптировать генерацию single-index направлений к ADP-контейнерам."""
...
def calculate_alpha_k(
    ...
    *,
    ...
    """Найти multi-index anisotropy ``alpha`` по локальной массе.

    В principal/orthogonal координатах строятся квадраты компонентов, после
    чего условие массы решается бисекцией. Для Epanechnikov применяется
    compact-support screening; ``tensor`` выбирает геометрию остатка.
    """
    ...
    utils.require(np.isfinite(N_loc) and N_loc > 0, "N_loc must be finite and positive")
    utils.require(
        tensor in {"orthogonal", "full"},
        "tensor must be 'orthogonal' or 'full'",
    )
    ...
    def enough(alpha: float) -> bool:
        """Проверить среднюю массу multi-index kernel при ``alpha``."""
...
def _compact_alpha(
    ...
    def enough(alpha: float, *, prune: bool = False) -> bool:
        """Проверить массу и при необходимости сжать активную поддержку."""
        mass = 0.0
...
def generate_multi_proj(
    ...
    """Сгенерировать единичные направления вокруг multi-index подпространства.

    Случайный вектор раскладывается на orthogonal и principal части,
    principal масштабируется ``sqrt(eigenvalues)``, затем части смешиваются
    коэффициентом ``alpha``. Выход имеет форму ``(J, P, d)``.
    """
    basis, eigenvalues = _prepare_basis_eigenvalues(basis, eigenvalues)
    utils.require(
        np.isfinite(alpha) and 0 <= alpha <= 1,
...
def generate_multi_proj_from_adp(
    ...
    """Адаптировать multi-index генерацию направлений к ADP-контейнерам."""
