"""Проверка форм, dtype и потоковых блоков общих ADP-вычислений."""
...
def require(
    ...
    """Поднять единый тип ошибки для проверки общей границы API."""
    if not condition:
...
def require_not_none(
    ...
    """Проверить обязательное значение и вернуть его с narrowed-типом."""
    if value is None:
        raise error_type(message)
    return value


def raise_import_error(message: str, cause: ImportError) -> NoReturn:
    """Пробросить единообразную ошибку отсутствующей optional-зависимости."""
