"""Общие CPU dense-моменты для legacy и manifold статистик ADP."""
...
class _DenseMoments:
    """Общие dense-моменты локальной ADP-статистики."""
...
def calculate_statistics(
    ...
    *,
    ...
    utils.require(isinstance(normalized, bool), "normalized must be boolean", TypeError)
...
def _dense_block(
    ...
    """Вычислить normalized/legacy ADP-моменты плотного блока.

    ``A`` уже нормирован по строкам; GEMM для всех ``(B,P)`` строк строит
    projections, после чего центрированные first moments дают ``I`` и ``U``.
    ``normalized`` определяет, возвращать ли моменты с внешней локальной
    массой.
    """
    moments = _dense_moments(Xc, Y, A, Phi, mass, normalized, include_eta=True)
    eta = utils.require_not_none(moments.eta, "dense moments did not compute eta")
    return (
        moments.I,
        moments.U,
        moments.mean,
...
def _dense_moments(
    ...
    *,
    ...
    ``A`` уже нормирован по строкам, ``Phi`` имеет форму ``(B,P,d)``.
    ``include_eta=False`` предназначен для manifold-пути, которому нужны только
    ``I/U`` и ``n_eff`` и не нужны диагностические residual ratios.
    """
    mean = A @ Xc
    y_bar = A @ Y
    ...
    Q *= A[:, None, :]
    summed = Q.sum(axis=2)
    I = Q @ Y - summed * y_bar[:, None]
    U = (Q.reshape(-1, len(Xc)) @ Xc).reshape(Phi.shape)
