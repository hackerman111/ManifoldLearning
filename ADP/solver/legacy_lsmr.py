...
from ..core.ADP_Solver import ADP_SolverResult
from ..core.ADP_Statistic import ADP_Statistics
from ..gpu import require_cupy


def _backend(U):
    """Выбрать NumPy или CuPy LinearOperator/LSMR по типу ``U``."""
    ...
    from cupyx.scipy.sparse.linalg import (  # pyright: ignore[reportMissingImports]
...
def _item(value):
    """Извлечь Python scalar из backend-specific scalar."""
...
def solve(
    ...
    *,
    ...
    """Решить legacy single/multi ADP задачу с backend-aware LSMR."""
...
def _solve_single(
    ...
    *,
    ...
    """Сделать alternating local-slope/global-index шаги single-index."""
    xp = backend[0]
...
def _solve_multi(
    ...
    *,
    ...
    """Сделать alternating шаги для ортонормированного multi-index basis."""
    xp = backend[0]
    I, U = statistics.I, statistics.U
    d = U.shape[2]
...
def _multi_coefficients(I, U, basis, ridge, xp=np):
    """Решить малую ridge-задачу локальных multi-index коэффициентов."""
    projected = U @ basis
...
def _solve_basis(
    ...
    *,
    ...
    """Решить глобальную basis-correction задачу matrix-free LSMR."""
    ...
    def matvec(vector):
        """Применить augmented forward action для multi-index basis."""
    ...
    def rmatvec(vector):
        """Применить adjoint augmented action для multi-index basis."""
...
def _canonical_basis(raw_basis, coefficients, xp=np):
    """Ортонормировать basis и канонизировать его через малую eigendecomposition."""
...
def _orient_columns(basis, xp=np):
    """Зафиксировать знаки столбцов basis для стабильной диагностики."""
...
def _slopes(
    ...
    """Оценить локальные ridge slopes при фиксированном индексе."""
    projected = U @ beta
...
def _solve_beta(
    ...
    *,
    ...
    """Решить single-index correction через augmented matrix-free LSMR."""
    xp, linear_operator, lsmr_method = backend
    rows = I.size
    d = U.shape[2]
    sqrt_lambda = math.sqrt(lambda_penalty)
    ...
    def matvec(vector):
        """Применить augmented forward action single-index correction."""
        data = (slopes[:, None] * (U @ vector)).ravel()
        return xp.concatenate((data, sqrt_lambda * vector))

    def rmatvec(vector):
        """Применить adjoint single-index correction operator."""
        data = vector[:rows].reshape(I.shape)
        return (
