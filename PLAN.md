...
updated_utc: 2026-09-23T14:42:55Z
...
| --- | --- | --- | --- |
| M1 | pending | Воспроизвести baseline на текущем checkout; выписать точные функции, формы, цели, один hot target/iteration и reference для обеих задач. | Таблица входов/параметров и paired outputs; зафиксированы версия/dirty tree, solver status, residual, projector distance и исходные wall-clock/RSS. |
| M2 | pending | Математически исследовать manifold: проверить минимум два точных кандидата, для отклоненных указать причину; сравнить цену построения, действия и recovery. | `docs/manifold_hpao_optimization_math.md`: формулы, допущения, сложность/байты, малые детерминированные dense/reference проверки и выбранный кандидат. Production-код не изменен. |
| M3 | pending | Математически исследовать HPAO/LSMR: проверить точные factorization/операторные или предобусловленные варианты против текущей correction-задачи и certificate. | Тот же документ: вывод `A/A*`, ridge-штрафа и сертификата, оценки matvec/итераций/памяти, малый direct-reference; выбранный кандидат. Production-код не изменен. |
| G | pending | Принять или отвергнуть каждого кандидата отдельно. | Для принятого варианта есть доказательство сохранения цели, классификация, reference error и ожидаемый выигрыш на текущем профиле; решение и ограничения зафиксированы в `DECISIONS.md`/`STATE.md`. До этого шага кодовую оптимизацию не начинать. |
| C1 | pending | Минимально реализовать принятый manifold-вариант, сохранив старый reference путь для сравнения. | Локальный dense/reference тест, degenerate/rank cases, residual и projector checks; парный benchmark hot path и полного `fit`, wall-clock и RSS. |
| C2 | pending | Минимально реализовать принятый HPAO-вариант для CPU, сохранив контракт общего operator и проверив затронутые backend/callers. | Adjoint identity, direct ridge/reference, normal certificate, convergence/failure tests; парный benchmark solver и полного single/multi `fit`, wall-clock и RSS. |
| V | pending | Проверить оба изменения вместе на base/J2 и еще одной форме по `d` или `P`; обновить затронутые маршруты `agent-notes` и отчет. | Повторные измерения в одинаковой среде, медиана/разброс, stop reason/качество/ошибка, peak RSS, тесты затронутых и общих путей; `PLAN.md`/`STATE.md` обновлены. |
